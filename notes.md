...
Strings are implemented as a collection of bytes. Rust only has one string type in the core language, which is the string slice `str` that is usually seen in its borrowed form `&str`. 
...
// also can load on a literal directly
let s = "initial contents".to_string();

// also equivalent to doing this
// let s = String::from("initial contents");
```

## Updating a String
Can use the + operator or the `format!` macro to concatenate `String` values.

### Appending to a String with `push_str` to a string slice
```rust
let mut s = String::from("foo");
s.push_str("bar"); # foobar

// add only one letter with push
...
In short, Rust doesn't allow us to index into a `String`.


## Slicing Strings
Rather than indexing with [] with a singular number, you can use [] with a range to create a string slice.
```rust
